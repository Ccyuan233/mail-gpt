...
def inspect_status(config, mailbox_factory=IMAPClient):
    ...
    if config.database.exists():
        ...
        try:
            ...
            if not since and db.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='settings'").fetchone():
    ...
    with mailbox_factory(config, since=since) as mailbox:
        for mail in mailbox.candidates():
